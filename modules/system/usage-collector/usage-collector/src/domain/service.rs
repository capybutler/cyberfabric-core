...
use usage_collector_sdk::{
    AggregationQuery, AggregationResult, AllowedMetric, ModuleConfig, Page, RawQuery,
    UsageCollectorPluginClientV1, UsageCollectorPluginSpecV1, UsageRecord, UsageCollectorError
};
...
impl Service {
    ...
    pub fn get_module_config(
        &self,
        module_name: &str,
    ) -> Result<ModuleConfig, DomainError> {
    ...
    /// Run an aggregated query against the storage plugin.
    ///
    /// # Errors
    ///
    /// Same shape as [`Service::create_usage_record`] — circuit-open, timeout,
    /// plugin error, or plugin-resolution error.
    ...
    /// Run a raw page query against the storage plugin.
    ///
    /// # Errors
    ///
    /// Same shape as [`Service::create_usage_record`] — circuit-open, timeout,
    /// plugin error, or plugin-resolution error.
    pub async fn query_raw(&self, query: RawQuery) -> Result<Page<UsageRecord>, DomainError> {
