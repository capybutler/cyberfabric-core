...
use usage_collector_sdk::{
    AggregationFn, AggregationQuery, AggregationResult, UsageCollectorError, Page, RawQuery,
...
async fn create_usage_record_delegates_to_plugin() {
    ...
    assert!(svc.create_usage_record(record(Uuid::new_v4())).await.is_ok());
}
...
#[tokio::test]
async fn query_aggregated_delegates_to_plugin() {
    let svc = make_service();
    let result = svc.query_aggregated(minimal_agg_query()).await;
    assert!(result.is_ok());
    assert_eq!(result.unwrap(), vec![]);
}

#[tokio::test]
async fn query_raw_delegates_to_plugin() {
    let svc = make_service();
    let result = svc.query_raw(minimal_raw_query()).await;
    assert!(result.is_ok());
...
#[tokio::test]
async fn query_aggregated_plugin_unavailable_returns_error() {
    let instance_id = format!(
        "{}test.mock._.svc_test.v1",
        UsageCollectorPluginSpecV1::gts_schema_id()
    );
    let hub = Arc::new(ClientHub::default());
    let instance = make_test_instance(&instance_id, plugin_content(&instance_id, "cyberfabric"));
    ...
    // plugin client NOT registered
    let svc = make_service_with_vendor(hub, "cyberfabric");
    let err = svc.query_aggregated(minimal_agg_query()).await.unwrap_err();
    assert!(matches!(err, DomainError::PluginUnavailable { .. }));
}

#[tokio::test]
async fn query_aggregated_circuit_open_returns_error() {
    let svc = make_cb_service(
        Arc::new(FailPlugin),
        2,
        Duration::from_secs(10),
...
#[tokio::test]
async fn query_raw_circuit_open_returns_error() {
    let svc = make_cb_service(
        Arc::new(FailPlugin),
        2,
        Duration::from_secs(10),
    ...
    let err = svc.query_raw(minimal_raw_query()).await.unwrap_err();
    assert!(matches!(err, DomainError::CircuitOpen));
}

#[tokio::test]
async fn query_aggregated_scope_propagated_to_plugin() {
    ...
    svc.query_aggregated(minimal_agg_query())
        .await
...
/// Repeated query timeouts must trip the circuit breaker.
#[tokio::test]
async fn query_timeout_trips_circuit_breaker() {
    let threshold = 2u32;
    let instance_id = format!(
        "{}test.mock._.query_timeout_cb.v1",
        UsageCollectorPluginSpecV1::gts_schema_id()
    );
    let hub = hub_with_plugin(&instance_id, "cyberfabric", Arc::new(SlowPlugin));
    let svc = Service::new(
    ...
    assert!(
        matches!(err, DomainError::CircuitOpen),
        "circuit should be open after {threshold} timeouts, got {err:?}"
    );
}
