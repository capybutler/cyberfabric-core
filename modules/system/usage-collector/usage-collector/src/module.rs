...
/// exposes `dyn UsageCollectorClientV1` for outbox delivery, and provides a gRPC server
/// for out-of-process module binaries.
#[modkit::module(
...
impl Module for UsageCollectorModule {
    ...
    async fn init(&self, ctx: &ModuleCtx) -> anyhow::Result<()> {
        let cfg: UsageCollectorConfig = ctx.config()?;
