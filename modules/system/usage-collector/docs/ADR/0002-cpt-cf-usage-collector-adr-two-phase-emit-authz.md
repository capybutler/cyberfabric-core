...
* Code review: all `emit()` call sites pass an `EmitAuthorization` obtained from `authorize_emit()` in the same request handler, never reused across request boundaries
...
To minimize exposure, `EmitAuthorization` MUST NOT be cached or reused across request handlers. Code review is the enforcement mechanism (see Confirmation).
