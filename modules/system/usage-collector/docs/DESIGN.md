...
| `AuthorizedUsageEmitter` | Time-limited authorization handle returned by `authorize_for()` / `authorize()`; carries the PDP permit result, the authorized `tenant_id`/`resource_id`/`resource_type`, the allowed-metrics list for the source module, and a monotonic issuance timestamp. `enqueue()` verifies the token has not exceeded its maximum age before proceeding. | `usage-emitter` crate |
...
- `AuthorizedUsageEmitter::build_usage_record(metric, value).enqueue()` — called inside the caller's DB transaction; verifies the token has not exceeded its maximum age, validates metric semantics (rejects counter records with a negative value or a missing idempotency key), verifies the metric is in the allowed list, captures subject ID and type from SecurityContext, then evaluates all `AuthorizedUsageEmitter` constraints in-memory before calling `Outbox::enqueue()` within the caller's transaction; rejects before the outbox INSERT on any failure
...
| `.enqueue()` (on builder) | Usage source (within transaction) | Verifies token freshness, validates metric semantics (rejects counter records with a negative value or a missing idempotency key), verifies metric is in allowed list, captures subject from SecurityContext, stamps outbox row with the bound `tenant_id`/`resource_id`/`resource_type`; calls `Outbox::enqueue()` within the caller's transaction on success; returns error without INSERT on any failure |
...
sequenceDiagram
    ...
    AE->>AE: capture subject_id / subject_type from SecurityContext
