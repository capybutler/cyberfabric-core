...
<!-- STATUS: IN_PROGRESS — all p1 DoD items implemented; one p1 instruction gap (inst-authz-1: no-open-transaction assertion deferred — requires modkit-db framework API change). BREAKING CHANGE — usage-emitter public surface refactored into the 3-layer Runtime/Factory/Emitter model per RESEARCH-emitter-runtime.md: layer 3 emitter struct renamed to `UsageEmitter`; the layer-1 trait is now `UsageEmitterRuntimeV1` (concrete `UsageEmitterRuntime` struct); the old factory-as-authorizer role is replaced by `UsageEmitterFactory::with_*().authorize(subject)` builder chain. ClientHub now exposes `dyn UsageEmitterRuntimeV1`. Design ID `cpt-cf-usage-collector-feature-sdk-and-ingest-core` is unchanged (no `-v2` suffix); the design-element rename is textual only and no checkbox state is altered by this phase. See the legacy-to-new mapping table in RESEARCH-emitter-runtime.md for the exact symbol substitutions. -->

- **Type**: Service

- **Purpose**: Establish the core data model, delivery traits, three-layer Runtime/Factory/Emitter architecture with two-phase authorization, transactional outbox pipeline, gateway ingest handler, static metric configuration, and no-op storage plugin. Delivers the complete in-process emission path from the factory's `.with_*().authorize(subject)` step through the outbox background pipeline to the gateway and plugin. This is the prerequisite for all other features.
...
- **Domain Model Entities**:
  ...
  - `UsageEmitter` (layer 3 of the Runtime/Factory/Emitter model — see RESEARCH-emitter-runtime.md for the legacy-to-new symbol mapping)
...
- **Design Components**:
  ...
  - [ ] `p1` - `cpt-cf-usage-collector-component-emitter`
    > **Owner**: F1 — initialises and configures the emitter (registers `dyn UsageEmitterRuntimeV1` in `ClientHub`; vends module-scoped `UsageEmitterFactory` instances via `runtime.factory(MODULE_NAME)`; the factory's `.with_*().authorize(subject)` chain produces per-call `UsageEmitter` handles)
    > **Used by**: F5 (type validation inside `UsageEmitterFactory::authorize`), F6 (SDK-side quota check inside `UsageEmitterFactory::authorize`)
...
- **Scope**:
  - `usage-emitter`: fetch registered usage type schema from types-registry inside the factory's `.authorize(...)` step (phase 1); validate the record against the schema in-memory before the outbox INSERT; surface validation failures as `UsageEmitterError` before any domain operation is committed
...
- **Design Components**:
  ...
  - [ ] `p1` - `cpt-cf-usage-collector-component-emitter` (type validation inside `UsageEmitterFactory::authorize`)
...
- **Phases/Milestones**:
  - Phase 1: Emitter integration — types-registry fetch and in-memory schema validation inside the factory's `.authorize(...)` step (phase 1 of the two-phase model)
...
- **Purpose**: Prevent outbox flooding by enforcing per-source emission quotas inside the factory's `.authorize(...)` step (phase 1 of the two-phase model) before any transaction opens. Supplement with per-(source, tenant) rate limit enforcement at the gateway on ingest to protect the storage backend from quota-exhausting sources.

- **Depends On**: `cpt-cf-usage-collector-feature-sdk-and-ingest-core`

- **Scope**:
  - `usage-emitter` phase 1 extension: the factory's `.authorize(...)` step fetches the current source-level emission quota and window snapshot from the gateway; `enqueue()` evaluates the quota in-memory and rejects before the outbox INSERT if the source quota is exhausted
...
- **Design Components**:
  ...
  - [ ] `p1` - `cpt-cf-usage-collector-component-emitter` (SDK-side quota check inside `UsageEmitterFactory::authorize`)
...
- **Sequences**:
  ...
  - `cpt-cf-usage-collector-seq-emit` (quota check in phase 1)  <!-- Shared Sequence Participation: F6 role = rate-gate (throttles emission rate — rejects inside UsageEmitterFactory::authorize (phase 1) when source quota is exhausted before outbox INSERT) -->
...
- **Phases/Milestones**:
  - Phase 1: Emitter-side quota check — extend the factory's `.authorize(...)` step with quota fetch and in-memory evaluation
...
- `cpt-cf-usage-collector-feature-type-system` requires `cpt-cf-usage-collector-feature-sdk-and-ingest-core`: type validation integrates into the factory's `.authorize(...)` step (phase 1 of the two-phase model) and gateway ingest handler established in Feature 1; independent of query and plugin features.
- `cpt-cf-usage-collector-feature-rate-limiting` requires `cpt-cf-usage-collector-feature-sdk-and-ingest-core`: quota checks extend the factory's `.authorize(...)` step (phase 1) and gateway ingest handler from Feature 1; independent of query and plugin features.
