...
#[test]
fn build_setter_called_twice_uses_last_value() {
    ...
    let ts1 = DateTime::parse_from_rfc3339("2025-01-01T00:00:00Z")
        .unwrap()
        .with_timezone(&Utc);
    let ts2 = DateTime::parse_from_rfc3339("2025-06-15T12:34:56Z")
        .unwrap()
        .with_timezone(&Utc);

    let record = UsageRecordBuilder::new()
        .with_module("first-module")
        .with_module("second-module")
        .with_tenant_id(first_tenant)
        .with_tenant_id(second_tenant)
        .with_metric("test.gauge", UsageKind::Gauge)
        .with_metric("test.counter", UsageKind::Counter)
        .with_value(1.0)
        .with_value(2.0)
        .with_resource(first_resource, "first.resource")
        .with_resource(second_resource, "second.resource")
        .with_subject(Subject::with_type(first_subject, "first.subject"))
        .with_subject(Subject::with_type(second_subject, "second.subject"))
        .with_idempotency_key("first-key")
        .with_idempotency_key("second-key")
        .with_timestamp(ts1)
        .with_timestamp(ts2)
        .build()
        .expect("required fields populated");

    assert_eq!(record.module, "second-module");
    assert_eq!(record.tenant_id, second_tenant);
    assert_eq!(record.metric, "test.counter");
    assert_eq!(record.kind, UsageKind::Counter);
    assert!((record.value - 2.0).abs() < f64::EPSILON);
    assert_eq!(record.resource_id, second_resource);
    assert_eq!(record.resource_type, "second.resource");
    assert_eq!(
        record.subject,
        Some(Subject::with_type(second_subject, "second.subject"))
    );
    assert_eq!(record.idempotency_key, "second-key");
    assert_eq!(record.timestamp, ts2);
}
