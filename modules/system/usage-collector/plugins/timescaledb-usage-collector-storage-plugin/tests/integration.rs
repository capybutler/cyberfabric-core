...
//!
//! Run with:
//!   cargo test -p cf-timescaledb-usage-collector-storage-plugin --features integration
...
use timescaledb_usage_collector_storage_plugin::domain::query_port::QueryPort;
...
use timescaledb_usage_collector_storage_plugin::infra::pg_query_port::PgQueryPort;
...
async fn query_raw_filter_subject_type() {
    ...
    assert_eq!(page.items.len(), 1);
    assert_eq!(page.items[0].subject_type, Some("user".to_owned()));
}
